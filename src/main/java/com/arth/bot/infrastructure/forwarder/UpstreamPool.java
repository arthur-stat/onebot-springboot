package com.arth.bot.infrastructure.forwarder;
...
public class UpstreamPool {
    ...
    final class UpstreamClient implements WebSocketHandler {
        ...
        public void handleMessage(WebSocketSession session, WebSocketMessage<?> message) {
            WebSocketSession onebotSession = sessionRegistry.get(selfId);

            if (onebotSession != null && onebotSession.isOpen()) {
                try {
                    onebotSession.sendMessage(new TextMessage(message.getPayload().toString()));
                } catch (IOException e) {
                    log.error("[forward] failed to forward message to session {}", selfId, e);
                }
            } else {
                log.warn("[forward] return session not found or closed, session id = {}", selfId);
            }
        }
